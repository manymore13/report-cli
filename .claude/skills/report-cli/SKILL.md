...
- `beginTime` = **固定 `2025-01-01`**（覆盖足够长的时间范围）
- `endTime` = **系统当前日期**（从系统上下文获取，动态计算），格式 `YYYY-MM-DD`
- 用户明示日期范围时，以用户指定的为准
...
```
https://reportapi.eastmoney.com/report/list?pageSize=20&pageNo=1&beginTime=2025-01-01&endTime={今天}&qType=1&industryCode=1046&industry=*&rating=*
```
...
| `pageNo` | 页码，从 1 开始 | 1 |
| `beginTime` | 开始日期，固定 2025-01-01 | 2025-01-01 |
| `endTime` | 结束日期，系统当前日期 | 动态计算 |
| `qType` | 固定 1 | 1 |
...
curl -s -X POST https://reportapi.eastmoney.com/report/list2 \
  ...
  -d '{"pageSize":20,"pageNo":1,"beginTime":"2025-01-01","endTime":"{今天}","code":"600519"}'
```
...
```
https://reportapi.eastmoney.com/report/jg?pageSize=20&pageNo=1&beginTime=2025-01-01&endTime={今天}&qType={qType}
```
