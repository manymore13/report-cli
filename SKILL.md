...
**从系统上下文获取当前日期（本机时间），动态计算，禁止写死。**

- `endTime` = 系统当前日期，格式 `YYYY-MM-DD`
- `beginTime` = 系统当前日期 - 30 天，格式 `YYYY-MM-DD`
- 用户明示日期范围时，以用户指定的为准

> 示例：若系统日期为 2026-05-25，则 `endTime=2026-05-25`，`beginTime=2026-04-25`。
...
| `pageNo` | 页码，从 1 开始 | 1 |
| `beginTime` | 开始日期，默认系统日期-30天 | 动态计算 |
| `endTime` | 结束日期，默认系统日期（今天） | 动态计算 |
| `qType` | 固定 1 | 1 |
